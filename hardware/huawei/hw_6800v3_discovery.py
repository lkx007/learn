...
def usage():
  print >> sys.stderr, "Usage: hwv3_discovery.py [--debug] --clusters <svc1>[,<svc2>...] --user <username> --password <pwd> --hostname  <hostname> --serial <serial> --discovery-types <type1>,[type2]"
...
for cluster in clusters:
  ...
  for discovery in DISCOVERY_TYPES:
    ...
    if discovery == 'base':
      data = conn.EnumerateInstances('HuaSy_ArrayProduct')
      for  x in data:
        if x['Name'].startswith(serial):
          output.append('{ "{#TYPE}":"%s","{#NAME}":"%s","{#ID}":"%s" }' % (discovery,x['ElementName'],x['Name']) )

    if discovery == 'baseOther':
