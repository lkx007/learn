...
class Hwv3:
	...
	def msg(self):
		arr2 = ['fcportMsg','poolMsg','enclosureMsg','raidMsg','diskMsg','diskOtherMsg','hostMsg','hostGroupMsg','LunMsg','LunGroupMsg','baseMsg','baseSizeMsg','baseOtherMsg']
		t2 = []
		for i in arr2:
			fun = 'self.' + i
	...
	def send_to_zabbix(self):
		tempFile = '/tmp/' + self.serial + self.fun
		f = open(tempFile,'w')
	...
	def poolMsg(self):
		...
		for x in data:
			serial = x['InstanceID'].decode()
			id = x['ElementName'].decode()
			newData[serial] = id
		...
		if self.fun == 'msg':
			...
			for k,cs_path in enumerate(cs_paths):
				serial  = cs_path['InstanceID'].decode()
				id = newData[serial]
				if serial.startswith(self.serial):
				...
				if name not in names:  # 
					...
					for s in source:
						if s['InstanceID'] == oid:
							...
							if dmsg:
								self.composeContent('storagehost.hostgroup.desc[{id}]'.format(id=id), dmsg[0]['ElementName']) #所属主机组
	...
	def hostGroupMsg(self):
		...
		for x in data:
			...
			if serial.startswith(self.serial):
				...
				self.composeContent('storagehg.desc[{id}]'.format(id=id), name) #  主机组名称
				...
				for s in source:
					if s['InstanceID'] == id:
						...
						self.composeContent('storagehg.mapped.hosts[{id}]'.format(id=id), json.dumps(list(tmp))) #  主机组下所有主机
		del data
		del source
		gc.collect()
	...
	# 			dmsg = self.conn.Associators(cs_inst.path,'HuaSy_OrderedMemberOfCollection','HuaSy_DeviceMaskingGroup')
	# 			if len(dmsg)>0:
	# 				self.composeContent('lun.mapped.lungroup[{id}]'.format(id=cs_path['DeviceID']), dmsg[0]['ElementName']) # lun 所在 lun组
	...
	# 			dmsg = self.conn.Associators(cs_inst.path,'HuaSy_AllocatedFromStoragePool','HuaSy_ConcreteStoragePool')
	# 			if dmsg:
	# 				self.composeContent('lun.pool.desc[{id}]'.format(id=cs_path['DeviceID']), dmsg[0]['ElementName'])#lun所属存储池
	...
	def baseOtherMsg(self):
		...
		for x in data:
			...
			if id.startswith(self.serial):
				if self.fun == 'msg':
					self.composeContent('dev.syslocation', x['ElementName']) # 
				else:
					self.composeContent('dev.status', x['HealthState']) # 
	...
	def baseMsg(self):
		...
		for x in data:
			...
			if id.startswith(self.serial):
				self.composeContent('dev.serial', x['IdentifyingNumber']) # 
				self.composeContent('dev.desc', x['ElementName']) # 
				self.composeContent('dev.version', x['Version']) # 
				self.composeContent('dev.mfc', x['Vendor']) #
	...
	def baseSizeMsg(self):
		...
		for x in data:
			...
			if id.startswith(self.serial):
				self.composeContent('dev.storage.size.total', x['TotalManagedSpace']) #总容量 
				self.composeContent('dev.storage.size.free', x['RemainingManagedSpace']) # 剩余容量
				self.composeContent('dev.storage.size.used', x['TotalManagedSpace']-x['RemainingManagedSpace']) #使用容量
