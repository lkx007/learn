...
def usage():
  print >> sys.stderr, "Usage: svc_perf_discovery_sender_zabbix.py [--debug] --clusters <svc1>[,<svc2>...] --user <username> --password <pwd> --hostname  <hostname> --smis_port 5989 --discovery-types <type1>,[type2]  "
  print >> sys.stderr, "Discovery types: 'volume-mdisk','volume','mdisk','pool'"


try:
  opts, args = getopt.gnu_getopt(sys.argv[1:], "-h", ["help", "clusters=", "user=", "password=","hostname=", "debug","smis_port=", "discovery-types="])
...
def getHostName(data,type):
  ...
  if len(data):
    for  x in data:
      output.append('{ "{#TYPE}":"%s","{#NAME}":"%s","{#ID}":"%s" }' % (type,x['InstanceID'],x['StorageID']) )
  return output  
...
def getDiskName(data,type):
  ...
  if len(data):
    for  x in data:
      ...
      output.append('{ "{#TYPE}":"%s","{#NAME}":"%s","{#ID}":"%s" }' % (type,x['Location'],x['DeviceID']) )
  return output 


def getBatteryName(data,type):
  output = []
  if len(data):
    for  x in data:
      output.append('{ "{#TYPE}":"%s","{#NAME}":"%s","{#ID}":"%s" }' % (type,x['ElementName'],x['DeviceID']) )
...
for cluster in clusters:
  ...
  conn = pywbem.WBEMConnection('https://'+cluster+':'+smis_port, (user, password), 'root/hitachi/smis',None,None,None,True)
  conn.debug = True
  ...
  for discovery in DISCOVERY_TYPES:
    ...
    if discovery == 'host':
      data = conn.EnumerateInstances('HITACHI_StorageHardwareID')
