...
class Hwv3:
	...
	def getLunInstance(self):
		self.lunInstance = []
		lunInstances = self.conn.EnumerateInstanceNames('HuaSy_StorageVolume')
		for x in lunInstances:
			if x['DeviceID'].startswith(self.serial):
				self.lunInstance.append(x)
		...
		for x in data:
			...
			if id.startswith(self.serial):
				if self.fun == 'msg':
					...
					self.composeContent('dev.wwn', x['OtherIdentifyingInfo'])
				else:
	...
	def baseMsg(self):
		...
		for x in data:
			...
			if id.startswith(self.serial):
				...
				self.composeContent('dev.name', x['ElementName']) # 
