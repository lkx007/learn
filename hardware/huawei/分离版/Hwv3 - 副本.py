		...
		for x in data:
			...
			if id.startswith(self.serial):
				if self.fun == 'msg':
					...
					self.composeContent('raid.size.free[{id}]'.format(id=id), x['RemainingManagedSpace']) #已使用容量
					self.composeContent('raid.size.used[{id}]'.format(id=id), x['TotalManagedSpace'] - x['RemainingManagedSpace'] ) #
