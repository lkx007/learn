...
def usage():
  print (sys.stderr, "Usage: hwv3_discovery.py  --clusters <svc1>[,<svc2>...] --user <username> --password <pwd> --hostname  <hostname>   --discovery-types <type1>,[type2]")
#python ./hwv3_discovery.py  --clusters '10.142.88.7' --user 'smis_admin' --password 'Admin@12' --hostname  'test' --discovery-types 'pool'

try:
  opts, args = getopt.gnu_getopt(sys.argv[1:], "-h", ["help", "clusters=", "user=", "password=","hostname=", "debug", "discovery-types=","proxy="])
...
if not user or not password:
  print(sys.stderr, '--user , --password options must be set')
  usage()
  sys.exit(2)
